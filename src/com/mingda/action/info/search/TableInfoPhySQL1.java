...
public class TableInfoPhySQL1 {
	...
	public StringBuffer getphysql(String tselect, String tfrom, String twhere,
		...
		if (ssql.indexOf("INFO_T_FAMILYCLASS.CLASSTYPE") > 0) {
			String bb = ssql.substring(0, ssql.lastIndexOf("where") + 5)
					+ " INFO_T_MEMBER.FAMILY_ID=INFO_T_FAMILY.FAMILY_ID and INFO_T_FAMILYCLASS.FAMILY_ID=INFO_T_FAMILY.FAMILY_ID and 1=1 and  "
					+ ssql.substring(ssql.lastIndexOf("where") + 5);
			ssql = bb;
		}
		shtml.append(ssql);
		// log.debug("xiu物理SQL语句:"+ssql);
