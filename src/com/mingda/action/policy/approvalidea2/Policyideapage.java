...
import com.mingda.action.policy.approval2.Policycheckpage;
import com.mingda.action.policy.approvalidea.Policyideasql;
...
public class Policyideapage {
	...
	public StringBuffer getCheckIdeaSelect(String sname,String stabid,String sonecheck){
		...
		//
		if("0".equals(stabid)){				//新增
			shtml.append(iok);
			shtml.append(inot);
			if(!"1".equals(sonecheck)){	//第一级审批机构不能选择(重新审批)
				shtml.append(ire);
			}
		}else if("1".equals(stabid)){		//调整复查
			shtml.append(iok);
			shtml.append(inot);
			if(!"1".equals(sonecheck)){	//第一级审批机构不能选择(重新审批)
				shtml.append(ire);
			}
		}else if("2".equals(stabid)){		//不符合标准
			if(!"1".equals(sonecheck)){	//第一级审批机构不能选择(重新审批)
				shtml.append(ire);
			}
			shtml.append(idel);
		}else if("3".equals(stabid)){		//重新审批
			shtml.append(iok);
			shtml.append(inot);
			if(!"1".equals(sonecheck)){	//第一级审批机构不能选择(重新审批)
				shtml.append(ire);
