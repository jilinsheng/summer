...
import com.mingda.entity.ImplTPaperinfo2;

/**
 * A data access object (DAO) providing persistence and search support for
 * ImplTPaperinfo2 entities. Transaction control of the save(), update() and
 * delete() operations can directly support Spring container-managed
 * transactions or they can be augmented to handle user-managed Spring
 * transactions. Each of these methods provides additional information for how
 * to configure it for the desired type of transaction control.
 * 
 * @see com.mingda.entity.ImplTPaperinfo2
 * @author MyEclipse Persistence Tools
 */

public class ImplTPaperinfo2DAO extends BaseHibernateDAO {
	private static final Log log = LogFactory.getLog(ImplTPaperinfo2DAO.class);
	// property constants
	...
	public void save(ImplTPaperinfo2 transientInstance) {
		log.debug("saving ImplTPaperinfo2 instance");
		try {
	...
	public void delete(ImplTPaperinfo2 persistentInstance) {
		log.debug("deleting ImplTPaperinfo2 instance");
		try {
	...
	public ImplTPaperinfo2 findById(Long id) {
		log.debug("getting ImplTPaperinfo2 instance with id: " + id);
		try {
			ImplTPaperinfo2 instance = (ImplTPaperinfo2) getSession().get(
					"com.mingda.entity.ImplTPaperinfo2", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(ImplTPaperinfo2 instance) {
		log.debug("finding ImplTPaperinfo2 instance by example");
		try {
			List results = getSession().createCriteria(
					"com.mingda.entity.ImplTPaperinfo2").add(
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding ImplTPaperinfo2 instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from ImplTPaperinfo2 as model where model."
					+ propertyName + "= ?";
	...
	public List findAll() {
		log.debug("finding all ImplTPaperinfo2 instances");
		try {
			String queryString = "from ImplTPaperinfo2";
			Query queryObject = getSession().createQuery(queryString);
			return queryObject.list();
		} catch (RuntimeException re) {
	...
	public ImplTPaperinfo2 merge(ImplTPaperinfo2 detachedInstance) {
		log.debug("merging ImplTPaperinfo2 instance");
		try {
			ImplTPaperinfo2 result = (ImplTPaperinfo2) getSession().merge(
					detachedInstance);
	...
	public void attachDirty(ImplTPaperinfo2 instance) {
		log.debug("attaching dirty ImplTPaperinfo2 instance");
		try {
	...
	public void attachClean(ImplTPaperinfo2 instance) {
		log.debug("attaching clean ImplTPaperinfo2 instance");
		try {
