		...
		<table width="100%" height="300" border="0" cellpadding="0"
			...
			<tr>
				<td width="120" align="left" valign="top"><jsp:include
						page="/page/info/memberchild/MemberChildInitServlet?nodeId=<%=nodeId%>&nodeName=<%=nodeName%>"
...
<!--
	function getChild(obj,code ,nodeId,oLevel){
		...
		var url = '/db/page/info/memberchild/MemberChildGetChildServlet';
...
<!--
	function contextMenu(code,childcode,nodeId){
    	...
    	if(code==null||code==""||code=="null"){
    		...
    		html+="<div style=\"cursor:hand;font-size:12px\" onMouseOver=\"this.style.backgroundColor='#FFCC99'\" onmouseout=\"this.style.backgroundColor='#CCCCCC'\" onclick =\"parent.editMemberChild('"+code+"','"+childcode+"','"+nodeId+"','edit')\">修改</div>";
    	}
    	
    	if(childcode==null||childcode==""||childcode=="null"){
    	}else{
    		html+="<div style=\"cursor:hand;font-size:12px\" onMouseOver=\"this.style.backgroundColor='#FFCC99'\" onmouseout=\"this.style.backgroundColor='#CCCCCC'\" onclick =\"parent.addMemberChildchild('"+code+"','"+childcode+"','"+nodeId+"','add')\">增加子项目</div>";
    	}
    	oPopup.document.body.innerHTML =html+"</div>"; 
   	 	oPopup.show(left, top, 60, 45, window.document.body);
	}
	//添加子结点-读取表单
	function addMemberChildchild(code,childcode,nodeId,type){
		var temp="新建子项目";
		alert(temp);
		if(code==""){
			o="";
        ...
        var pars="code="+code+"&nodeId="+nodeId+"&type="+type+"&childcode="+childcode;
        	alert(pars);
		var url = '/db/page/info//memberchild/MemberChildGetFormServlet';
        var myAjax = new Ajax.Request(
        url,
        	{
        		method: 'post',
	...
	//修改本节点-读取表单
	function editMemberChild(code,childcode,nodeId,type){
		alert("修改");
		o="";
		var pars="code="+code+"&nodeId="+nodeId+"&type="+type+"&childcode="+childcode;
		alert(pars);
	...
	function memberchildSave(){
		...
		var url = '/db/page/info//memberchild/MemberChildSaveServlet';
        var myAjax = new Ajax.Request(
        url,
        	{
        		method: 'post',
	...
	//左键点击显示信息
	function viewMemberChild(nodeId,nodeName){
        ...
        method: 'post',
        parameters: pars,
        onComplete: viewMemberChildResult
        });
     }
     function viewMemberChildResult(response){
