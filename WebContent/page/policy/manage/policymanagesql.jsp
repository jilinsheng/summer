...
<%	
	...
	//接收qsuppname
	String reqsuppname= request.getParameter("qsuppname");
	reqsuppname = new String(reqsuppname.getBytes("ISO8859_1"), "GB18030");//解码		
	if (reqsuppname == null) {
		//默认为空
		reqsuppname = "";    //空业务所属名称
	}
	//接收qnotpname
	String reqnotpname= request.getParameter("qnotpname");
	reqnotpname = new String(reqnotpname.getBytes("ISO8859_1"), "GB18030");//解码		
	if (reqnotpname == null) {
		//默认为空
		reqnotpname = "";    //空业务不属名称
	}
...
<html>
  <head>
	...
	<script type="text/javascript">	
		...
		var selid = "";		//编号
		var selname = "";	//名称
		var selsuppname = "";//业务所属名称(名称用逗号隔开)
		var selnotpname = "";//业务不属名称(名称用逗号隔开)
		...
		function GetPhySql(mode){		
			...
			//查询条件
		   	...
		   	//查询结果
		   	//家庭				
			selselect = "INFO_T_FAMILY.FAMILY_ID";
			//
		    ...
		    $.post("page/info/search/TableInfoServlet",      //服务器页面地址
		        {
                    ...
                    ssuperpolicy: selsuppname,	//所属业务
                    snotpolicy: selnotpname,	//不属业务
                    ttype: ttype		            
		        },
		        function(result) {   //回调函数
		        	if (result=="-1"){
		...
		//保存操作
		function SaveStandardSql(physql,locsql){
			var sphysql = "",slocsql = "";			
			//
			sphysql = physql;			
    		slocsql = locsql;    			
			//
		    if (!confirm("确定保存["+selname+"]业务标准条件?")) {
		        return;
		    }
			//
			//
		    $.post("page/policy/manage/PolicyManageServlet",            //服务器页面地址
		        {
		            action: "updatePolicySql",             //action参数
		            sid: selid,
		            sphysql: sphysql,
		            slocsql: slocsql        
		        },
		...
		function GetPolicySqlItem() {
		    ...
		    $.post("page/policy/manage/PolicyManageServlet",   //服务器页面地址
		        {
		            action: "getPolicySqlItem",             //action参数
		            sid: selid,
		            mode: mode         
		        },
		        function(result) {                    	//回调函数
		...
		function IniPage(){	
			...
			selid = "<%=reqid%>";    //编号 
			selname = "<%=reqname%>";    //名称
			selsuppname = "<%=reqsuppname%>";    //所属业务名称
			selnotpname = "<%=reqnotpname%>";    //不属业务名称
  		...
  		<table width="100%" border="0" cellspacing="0" cellpadding="0">	   		   		
	    	<tr align="center">
	    			...
	    			<table width="100%" border="0" cellspacing="0" cellpadding="0">	   		   		
				    	<tr align="center">
				    		<td valign="top" width="40%">
				    			<fieldset  class='list'>
				    			<legend  class='legend'>标准设置</legend>
	    		...
	    		<td valign="top" width="60%">	    		
	    			<fieldset  class='list'>
	    			<legend  class='legend'>[<%=reqname%>]标准属性</legend>  
	    				<table width="100%" border="0" cellspacing="0" cellpadding="0">
	    					<tr align="center" class = 'titles'>	
	    						<td valign="top">
	    							<input type="radio" name="rd" id="rdc" value="rdc" onclick="GetPolicySqlItem()" checked />选择条件语句
									<input type="radio" name="rd" id="rdp" value="rdp" onclick="GetPolicySqlItem()" />标准SQL语句
	    						</td>
	    					</tr>	    					
					    	<tr align="center">
					    		<td valign="top">
			          	...
			          	<div class = "titles">
						  	...
						  	<input type='button' value='重置' onclick="GetPolicySqlItem()"></input>
