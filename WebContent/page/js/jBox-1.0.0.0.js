...
*/
var jBox={
    imgs:['/db/page/images/minall.gif', '/db/page/images/closeall.gif', '/db/page/images/restore.gif','/db/page/images/resize.gif'],
	boxes: [],
	minimizeorder: 0,
	xiufun: "",//xiu
	init:function(id){
		...
		var box = $('<div class="jBox" id="jb'+id+'"></div' ).appendTo(holder);
            ...
            .append('<div class="jBoxControls" align="center"><button class = "btn" title="Close">关  闭 </button></div>')
	...
	open:function( id, type, source, title,attr,ajaxOpt){	
		//beg xiu
		//标题后面跟& 表示为关闭窗体后执行此函数(此函数多为刷新或者重新查询)
		var stitle = title.split("&");
		var silen = stitle.length;
		if(silen>1){			
			title = stitle[0];
			jBox.xiufun = stitle[1];			
		}
		//end xiu
