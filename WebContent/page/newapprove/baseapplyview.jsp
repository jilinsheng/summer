	...
	<table align="center" width="100%" cellspacing="0" cellpadding="0"
		...
		<tr>
			...
			<td colspan="5">(1).村级审批：<U><font color="red"><c:if
							...
							test="${pa.checkflag1 =='8'}">终止</c:if> <c:if
							test="${pa.checkflag3 =='9'}">不同意救助</c:if> </font></U>(2).乡镇审批：<U><font
					color="red"><c:if test="${pa.checkflag2 =='1'}">审批中</c:if> <c:if
							test="${pa.checkflag2 =='2'}">同意救助</c:if> <c:if
							...
							test="${pa.checkflag2=='8'}">终止</c:if> <c:if
							test="${pa.checkflag3 =='9'}">不同意救助</c:if> </font></U>(3).区县审批：<U><font
					color="red"><c:if test="${pa.checkflag3 =='1'}">审批中</c:if> <c:if
							test="${pa.checkflag3 =='2'}">同意救助</c:if> <c:if
							...
							test="${pa.checkflag3 =='8'}">终止</c:if> <c:if
							test="${pa.checkflag3 =='9'}">不同意救助</c:if> </font></U>
			</td>
		</tr>
		<tr>
			<th>审批金额</th>
			<td colspan="4"> <c:out value="${pa.checkmoney}"></c:out>元</td>
			<!-- <th>选择审批结果</th>
			...
			<td><button type="button" onclick="window.close()">关闭</button></td> 
		</tr>
	</table>
</body>
</html>