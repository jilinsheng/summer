...
<html>
  <head>
	...
	<script type="text/javascript">	
		...
		//保存操作
		function SaveValidateSql(physql,locsql){
			var sphysql = "",slocsql = "";			
			//
			sphysql = physql;			
    		slocsql = locsql;    			
			//
		    if (!confirm("确定保存["+selname+"]信息合法验证条件?")) {
		        return;
		    }
			//
			//
		    $.post("page/querymanage/QueryManageServlet",            //服务器页面地址
		        {
		            action: "updateValidateSql",             //action参数
		            sid: selid,
		            sphysql: sphysql,
		            slocsql: slocsql        
		        },
		...
		//取得信息合法验证SQL属性
		function getInfoValidateSqlItem() {
			var mode = "";
		    //清空条件
			ResetSql();
			//选择逻辑语句或者物理语句
		    ...
		    $.post("page/querymanage/QueryManageServlet",   //服务器页面地址
		        {
		            action: "getInfoValidateSqlItem",             //action参数
		            sid: selid,
		            mode: mode         
		        },
		        function(result) {                    	//回调函数
  		...
  		<table width="100%" border="0" cellspacing="0" cellpadding="0">	   		   		
	    	<tr align="center">
	    		<td valign="top" width="40%"  id = "matchtd">
	    			<fieldset  class='list'>
	    			<legend  class='legend'>信息合法验证条件设置</legend>
	    		...
	    		<td valign="top">
	    			<fieldset  class='list'>
	    			<legend  class='legend'>[<%=reqname%>]信息合法验证条件属性</legend>  
	    				<table width="100%" border="0" cellspacing="0" cellpadding="0">	
	    					<tr align="center" class = 'titles'>	
	    						<td valign="top">
	    							<input type="radio" name="rd" id="rdc" value="rdc" onclick="getInfoValidateSqlItem()" />选择条件语句
									<input type="radio" name="rd" id="rdp" value="rdp" onclick="getInfoValidateSqlItem()" checked />标准SQL语句
	    						</td>
	    					</tr>   		   		
					    	<tr align="center">
					    		<td valign="top" width="100%">
					    			<textarea name="sqlwhere" rows="8" id="sqlwhere" 
			          	...
			          	<div class = "titles">	
						  	...
						  	<input type='button' value='重置' onclick="getInfoValidateSqlItem()"></input>
